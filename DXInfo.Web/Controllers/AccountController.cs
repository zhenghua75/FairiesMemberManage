...
using DXInfo.Web.Providers;
using DXInfo.Web.Results;
using DataTables;
...
namespace DXInfo.Web.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public UserInfoViewModel GetUserInfo()
        {
            ...
            NameCode nc = UserManager.GetNameCode("AuthorityType", user.AuthorityType);
            Warehouse wh = UserManager.GetWarehouse(user.Id);
            Dept dept = UserManager.GetDept(user.Id);
            return new UserInfoViewModel
            {
                UserName = user.UserName,
                FullName = user.FullName,
                DeptName = dept ==null? "":dept.Name,
                WhName = wh==null ? "":wh.Name,
                AuthorityType = nc.Name,
        ...
        public async Task<ManageInfoViewModel> GetManageInfo(string returnUrl, bool generateState = false)
        {
            ...
            foreach (UserLoginInfo linkedAccount in UserManager.GetLogins(user.Id))
            {
        ...
        [Route("ResetPassword")]
        public async Task<IHttpActionResult> ResetPassword([FromBody]int UserId)
        {
            if (UserId == 0)
            {
                ModelState.AddModelError("other", "请选中需要重置密码的用户");
                return BadRequest(ModelState);
            }
            string resettoken = await UserManager.GeneratePasswordResetTokenAsync(UserId);
            IdentityResult result = await UserManager.ResetPasswordAsync(UserId,resettoken,"123456");
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var user = new ApplicationUser() { UserName = model.UserName , FullName =model.FullName, DeptId =model.DeptId, WhId = model.WhId, AuthorityType =model.AuthorityType};
            ...
            //return Ok();
            response.data.Add(new Dictionary<string, object> { { "Succeeded", result.Succeeded } });
            return Json(response);
        }
