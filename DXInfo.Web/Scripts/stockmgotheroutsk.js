...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (curmvid == '0') {
                ...
                mveditor.field('Vouch.BusType').update(optbus);
            } else {
                $('#isverify').val(json.data[0].Vouch.IsVerify);
                $('.panel .panel-title span').html(json.data[0].Vouch.Code);
                if (json.data[0].Vouch.BusType == '020' || json.data[0].Vouch.BusType == '021' || json.data[0].Vouch.BusType == '022') {
                    $('div.group1').remove();
                    $('div.group2').remove();
                    $('#mvunverify').remove();
                } else {
                ...
                if (json.data[0].Vouch.IsVerify == true) {
                    ...
                    if ($('#gparamauthtype').val()!='0' && (json.data[0].Vouch.BusType == '020' || json.data[0].Vouch.BusType == '021' || json.data[0].Vouch.BusType == '022')) {
                        $('#mvverify').addClass('disabled');
                        $('#mvunverify').addClass('disabled');
                    }
                }
    ...
    function createMvsTable(json) {
        var eletable = $('#stockmgotheroutsk');
        ...
        var strbustype = '';
        if (json != null) {
            strbustype = json.data[0].Vouch.BusType;
        }
        ...
        $.each(vouchsfields, function (key, val) {
            ...
            if ((strbustype == '020' || strbustype == '021' || strbustype == '022') && val.name == 'Vouchs.Num') {
                val.type = "readonly";
                vouchsfields[key].issubmit = false;
            }
                        ...
                        $.ajax({
                            ...
                            success: function (jsonmv) {
                                ...
                                .done(function (json) {
                                    ...
                                    document.location = "/StockMg/OtherOutStock?id=" + newvid + '&type=' + vouchtype;
                                })
        ...
        var strdom = "<'row'<'col-sm-4'B><'col-sm-4'><'col-sm-4'f>>" + "<'row'<'col-sm-12'rt>>" + "<'row'<'col-sm-4'i><'col-sm-8'p>>";
        var strbuttons = [
            { extend: "create", editor: mvseditor },
            { extend: "edit", editor: mvseditor },
            { extend: "remove", editor: mvseditor }
        ];
        if (strbustype == '020' || strbustype == '021' || strbustype == '022') {
            strbuttons = [
                { extend: "edit", editor: mvseditor }
            ];
            if (json.data[0].Vouch.IsVerify) {
                strdom = "<'row'<'col-sm-4'><'col-sm-4'><'col-sm-4'f>>" + "<'row'<'col-sm-12'rt>>" + "<'row'<'col-sm-4'i><'col-sm-8'p>>";
            }
        }
        table = $('#stockmgotheroutsk')
        ...
        if ($('#gparamisloctor').val() == 'true') {
            var loctoridnode = mvseditor.field('Vouchs.ToLocatorId').input();
