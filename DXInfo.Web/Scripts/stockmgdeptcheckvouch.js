...
var wizsteps = {
    tab1: {
        Id: 'tab1',
        Name: '盘点单',
        vouchurl: "/api/Editor/Data?model=Vouch&VouchType=016",
...
$(document).ready(function () {
    ...
    function createMvsTable(tabconf, json, formid, tableid) {
        ...
        tabconf.vouchseditor = new $.fn.dataTable.Editor({
            ...
            ajax: {
                create: function (method, url, data, successCallback, errorCallback) {
                    var vouchid = $('#curvid').val();
                    var vouchtype = $('#curtype').val();
                    var vouchbus=tabconf.voucheditor.field('Vouch.BusType').val();
                    if (vouchbus == '' || vouchbus == null) {
                        addUIAlter('nav.breadcrumb', '请选择盘点业务类型', 'error');
                        ...
                        $.ajax({
                            ...
                            success: function (jsonmv) {
                                ...
                                .done(function (json) {
                                    ...
                                    document.location = "/StockMg/DeptCheckVouch?id=" + newvid + '&type=' + vouchtype;
                                })
        ...
        if (tabconf.Id == 'tab1') {
            tabconf.vouchseditor.on('open', function (e, display, action) {
                if (action == 'create') {
                    var curbustype = tabconf.voucheditor.field('Vouch.BusType').val();
                    if (curbustype == '' || curbustype == null) {
                        addUIAlter('nav.breadcrumb', '请选择盘点业务类型', 'error');
                        tabconf.vouchseditor.close();
                        return false;
                    }
                    var optinv = [];
                    $.each(tabconf.vouchstable.ajax.json().options['Vouchs.InvId'], function (k, v) {
                        switch (curbustype) {
                            case '023':
                                var arrtmp=['QTZL','mjl','BC','YCL','stck','bcp'];
                                if ($.inArray(v.label.CategoryCode,arrtmp)==-1) {
                                    optinv.push(v);
                                }
                                break;
                            case '024':
                                if (v.label.CategoryCode=='bcp') {
                                    optinv.push(v);
                                }
                                break;
                            case '025':
                                var arrtmp = ['QTZL', 'mjl', 'BC', 'YCL'];
                                if ($.inArray(v.label.CategoryCode, arrtmp) >=0) {
                                    optinv.push(v);
                                }
                                break;
                        }
                    })
                    tabconf.vouchseditor.field('Vouchs.InvId').update(optinv);
                }
                if (action == 'edit') {
            ...
            case 'vouchsedit':
                $.ajax({
                    ...
                    success: function (json) {
                        if (tabconf.Id == 'tab1') {
                            $('#tab1').append('<blockquote>操作说明：' +
                                '<small class="c-red">请把所有同类型存货的盘点数全部添加完后，再进行审核，一旦审核，则未添加盘点数的同类型存货全部视为盘点数为0</small>' +
                                '</blockquote>');
                        }
                        if (json.data == undefined || json.data.length == 0) {
                        ...
                        $('#' + tabconf.Id).find('#mvverify').on('click', function () {
                            var vouchid = $('#' + tabconf.Id + 'mvform').data('id');
                            var vouchtype = tabconf.vchtype;
                            var bustype = tabconf.voucheditor.field('Vouch.BusType').val();
                            var objtarget = this;
                            panelLoading(objtarget);
                            $.ajax({
                                url: "/api/Editor/Data?model=Vouchs",
                                ...
                                success: function (jsonsk) {
                                    if (jsonsk.data == undefined) {
                                        ...
                                        reqdata.data['row_' + vouchid] = { Vouch: { IsVerify: true, VouchType: vouchtype, BusType: bustype } };
                                        $.ajax({
                                            url: "/api/Editor/Data?model=Vouch&VouchType=" + vouchtype,
                                            dataType: 'json',
                                            method: 'POST',
                                            ...
                                            success: function (jsonmv) {
                                                ...
                                                if (tabconf.Id == 'tab1') {
                                                    document.location = "/StockMg/DeptCheckVouch?id=" + jsonmv.data[0].DT_RowId.replace('row_', '') + "&type=" + jsonmv.data[0].Vouch.VouchType;
                                                }
                                            },
                                            error: function (jsonmv) {
                                                panelLoaded(objtarget);
                        ...
                        $('#' + tabconf.Id).find('#mvunverify').on('click', function () {
                            ...
                            reqdata.data['row_' + vouchid] = { Vouch: { IsVerify: false, VouchType: vouchtype, BusType: bustype } };
                            var objtarget = this;
                            panelLoading(objtarget);
                            $.ajax({
                                url: "/api/Editor/Data?model=Vouch&VouchType=" + vouchtype,
    ...
    function checkMvControlDis(tabconf, mod, formid) {
        switch (mod) {
            ...
            case '1':
                ...
                for (var i = 0; i < tabconf.vouchfields.length; i++) {
                    if (tabconf.vouchfields[i].isdisabled === undefined || tabconf.vouchfields[i].isdisabled == false) {
                        if (tabconf.vouchfields[i].name != 'Vouch.BusType') {
                            var inputtmp = $('#' + formid).find('#DTE_Field_' + tabconf.vouchfields[i].name.replace('.', '-'));
