...
var wizsteps = {
    tab1: {
        Id: 'tab1',
        Name: '货位调整单',
        vouchurl: "/api/Editor/Data?model=Vouch&VouchType=011",
    ...
    function createMvsTable(tabconf, json, formid, tableid) {
                        ...
                        $.ajax({
                            ...
                            success: function (jsonmv) {
                                ...
                                .done(function (json) {
                                    ...
                                    document.location = "/StockMg/AdjustLocatorVouch?id=" + newvid + '&type=' + tabconf.vchtype;
                                })
            ...
            if (action != 'remove') {
                ...
                if (iserror) {
                    ...
                    $.each(o.data, function (k, v) {
                        ...
                        if (o.data[k].Vouchs.FromLocatorId == '0' || o.data[k].Vouchs.FromLocatorId == '' || o.data[k].Vouchs.FromLocatorId == 'null') {
                            o.data[k].Vouchs.FromLocatorId = undefined;
                        }
                    })
                }
            }
        ...
        if (tabconf.Id == 'tab1') {
            ...
            $(invidnode).on('change', function () {
                tabconf.vouchseditor.field('Inventory.Specs').val('');
                tabconf.vouchseditor.field('UOM.Name').val('');
                $(tabconf.vouchseditor.field('Vouchs.Batch').input()).empty();
                tabconf.vouchseditor.field('CurStockNum').set('');
                tabconf.vouchseditor.field('Vouchs.Num').set('');
                tabconf.vouchseditor.field('Vouchs.MadeDate').set('');
                tabconf.vouchseditor.field('Vouchs.InvalidDate').set('');
                ...
                var loctor = tabconf.vouchseditor.field('Vouchs.FromLocatorId').val();
                if (loctor == '0' || loctor == null || loctor == '') {
                    loctor = 'is null';
                } else {
                    loctor = '=' + loctor;
                }
            ...
            var fromlocinput = tabconf.vouchseditor.field('Vouchs.FromLocatorId').input();
            $(fromlocinput).on('change', function () {
                tabconf.vouchseditor.field('Vouchs.InvId').set('');
                tabconf.vouchseditor.field('Inventory.Specs').val('');
                tabconf.vouchseditor.field('UOM.Name').val('');
                $(tabconf.vouchseditor.field('Vouchs.Batch').input()).empty();
                tabconf.vouchseditor.field('CurStockNum').set('');
                    ...
                    success: function (json) {
                        if (json.data == undefined || json.data.length == 0) {
                            if (mvid != '0' && tabconf.ismain == true) {
                                ...
                                var strempty = '<div class="col-sm-12 text-c emptytitle"><h1><i class="iconfont icon-jinzhi c-warning"></i>货位调整单未审核</h1></div>' +
                                    '<div class="col-sm-12 text-c emptycontent">' + tabconf.Name + '暂未生成，需等待货位调整单审核完成，请先审核货位调整单。</div>';
                        ...
                        $('#' + tabconf.Id).find('#mvverify').on('click', function () {
                            ...
                            $.ajax({
                                ...
                                success: function (jsonmv) {
                                    ...
                                    if (tabconf.Id == 'tab1') {
                                        document.location = "/StockMg/AdjustLocatorVouch?id=" + jsonmv.data[0].DT_RowId.replace('row_', '') + "&type=" + jsonmv.data[0].Vouch.VouchType;
                                    }
                                },
                                error: function (jsonmv) {
                                    panelLoaded(objtarget);
