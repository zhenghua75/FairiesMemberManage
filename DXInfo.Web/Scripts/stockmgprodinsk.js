...
var vouchsfields = [
    ...
    },
    {
        ...
        type: "select2",
        issubmit: true,
        optcond: { CategoryCode: ['BC', 'mjl', 'QTZL', 'stck', 'YCL'] },
        placeholder: "请选择...",
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            createMvsTable(json);
            if ($('#cursid').val() != '') {
                if ($('div.group4').hasClass('hide')) {
                    $('div.group4').removeClass('hide')
                }
            } else {
                if (!$('div.group4').hasClass('hide')) {
                    $('div.group4').addClass('hide')
                }
            }
            if (curmvid == '0') {
    ...
    function createMvsTable(json) {
        var eletable = $('#stockmgprodinsk');
            ...
            ajax: {
                create: function (method, url, data, successCallback, errorCallback) {
                    ...
                    if (vouchid == '0') {
                        ...
                        if ($('#cursid').val() != '') {
                            datatmp.Vouch.SourceId = $('#cursid').val();
                        }
                        ...
                        $.ajax({
                            ...
                            success: function (jsonmv) {
                                ...
                                .done(function (json) {
                                    ...
                                    var redurl = "/StockMg/ProductInStock?id=" + newvid + '&type=' + vouchtype;
                                    if ($('#cursid').val() != '') {
                                        redurl += '&sid=' + $('#cursid').val();
                                    }
                                    if ($('#optab').val() != '') {
                                        redurl += '&optab=' + $('#optab').val();
                                    }
            ...
            if (action != 'remove') {
                ...
                $.each(efields, function (k, v) {
                    if (v.errorinfo !== undefined) {
                        ...
                        if (!field.val() && !$(field.input()).val() && (v.name != 'Vouchs.MadeDate' && v.name != 'Vouchs.InvalidDate')) {
                            field.error(v.errorinfo);
                            iserror = true;
                        }
                        var dateformat = /^(\d{4})-(0\d{1}|1[0-2])-(0\d{1}|[12]\d{1}|3[01])$/;
                        if ($('#gparamisshlife').val() == 'true' && mvseditor.field('isShLife') == true) {
                            ...
                            $.each(o.data, function (key, val) {
                                if (o.data[key].Vouchs.MadeDate == null || o.data[key].Vouchs.MadeDate == '') {
                                    o.data[key].Vouchs.MadeDate = undefined;
                                }
                                if (o.data[key].Vouchs.MadeDate == null || o.data[key].Vouchs.MadeDate == '') {
                                    o.data[key].Vouchs.MadeDate = undefined;
                                }
        ...
        mvseditor.on('open', function (e, display, action) {
            if (action != 'remove') {
                ...
                if (action == 'edit') {
                    ...
                    mvseditor.field('Vouchs.MadeDate').set(madedate);
                    mvseditor.field('Vouchs.Batch').val(madedate.replace(/-/g, ''));
                }
            }
        })
        ...
        var backNext = [
                ...
                },
                {
                    label: '修改',
                    fn: function (e) {
                        checkmvseditorindex = table.row({ selected: true }).index();
        ...
        var drawcallback = function () {
            ...
            table.row(indexes[currentPosition]).select();
        }

        var strdom="<'row'<'col-sm-4 mvsbtns'B><'col-sm-4'><'col-sm-4'f>>" + "<'row'<'col-sm-12'rt>>" + "<'row'<'col-sm-4'i><'col-sm-8'p>>";
        ...
        $(invidnode).on('change', function () {
            ...
            for (var i = 0; i < invopts.length; i++) {
                if (invopts[i].value == curselectinvid) {
                    ...
                    mvseditor.field('isShLife').val(invopts[i].label.IsShelfLife);
                    var inputtmp = mvseditor.field('Vouchs.Price').input();
                    if (invopts[i].label.Price == '' || invopts[i].label.Price == null) {
                        $(inputtmp).attr('readonly', false);
                        $(inputtmp).removeClass('disabled');
                    } else {
                        mvseditor.field('Vouchs.Price').val(invopts[i].label.Price);
                        $(inputtmp).attr('readonly', true);
                        $(inputtmp).addClass('disabled');
                    }

                    if (invopts[i].label.IsShelfLife == true) {
                        if (mvseditor.field('Vouchs.MadeDate').val() == '') {
                            var today = new Date();
                            mvseditor.field('Vouchs.MadeDate').set(today);
                            mvseditor.field('Vouchs.Batch').val(moment().format('YYYY-MM-DD').replace(/-/g, ''));
        ...
        $('#stockmgprodinsk').on('click', 'a.editor_edit', function (e) {
            e.preventDefault();
            var eletr = $(this).closest('tr');
            if (eletr.hasClass('child')) {
                eletr = eletr.prev('tr.parent');
            }
            table.rows(eletr).select();
            mvseditor.edit(eletr, {
                title: '编辑记录',
                buttons: '修改'
            });
        });

        $('#stockmgprodinsk').on('click', 'a.editor_remove', function (e) {
            e.preventDefault();
            var eletr = $(this).closest('tr');
            if (eletr.hasClass('child')) {
                eletr = eletr.prev('tr.parent');
