...
$.extend( true, $.fn.dataTable.Editor.classes, {
	...
	"form": {
		"tag": "form form-horizontal",
		"button": "btn btn-secondary radius"
	},
	"field": {
		"wrapper": "DTE_Field",
		"label":   "col-xs-4 col-sm-3 form-label",
		"input":   "col-xs-8 col-sm-9 formControls",
...
DataTable.Editor.display.bootstrap = $.extend( true, {}, DataTable.Editor.models.displayController, {
	...
	"init": function ( dte ) {
		...
		dte.on( 'open.dtebs', function ( e, type ) {
			if ( type === 'inline' || type === 'bubble' ) {
				$('div.DTE input[type=text]').addClass( 'input-text' );
				$('div.DTE select').addClass( 'select' );
				$('div.DTE textarea').addClass( 'textarea' );
			}
		} );
	...
	"open": function ( dte, append, callback ) {
		...
		$('input:not([type=checkbox]):not([type=radio])', self._dom.content).addClass( 'input-text' );
		$('select', self._dom.content).addClass( 'select' );
		$('textarea', self._dom.content).addClass( 'textarea' );
	},
