...
function info ( api )
{
	...
	var add = function ( name, i18nname, num ) {
		output.append( $('<span class="select-item"/>').append( api.i18n(
			'select.'+name+'s',
			{ _: '已选中%d'+i18nname, 0: '', 1: '已选中1'+i18nname },
			num
		) ) );
	};

	add( 'row', '行',   api.rows( { selected: true } ).flatten().length );
	add( 'column', '列', api.columns( { selected: true } ).flatten().length );
	add( 'cell',  '单元格', api.cells( { selected: true } ).flatten().length );
