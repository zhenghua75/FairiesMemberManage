...
namespace DataTables
{
    ...
    abstract public class Query
    {
        ...
        internal List<string> _groupField = new List<string>();
        ...
        //internal string _groupClause = string.Empty;
        /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
         * Static methods
         */
        ...
        public Query Group(string field)
        {
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                if (!_groupField.Any(n => n == fields[i]))
                    _groupField.Add(fields[i]);
            }
        ...
        /// <returns></returns>
        public Query Group(IEnumerable<string> fields)
        {
            foreach (var field in fields)
            {
                Group(field);
        ...
        virtual protected string _BuildField(Boolean addAlias = false)
        {
            ...
            for (int i = 0; i < _field.Count; i++)
            {
                ...
                {
                    string[] ass = Regex.Split(field, " as ", RegexOptions.IgnoreCase);
                    field = ass[0].Trim() + " as " + _fieldQuote + ass[1].Trim()+ _fieldQuote;
                }

                if (addAlias && field.IndexOf(" as ") == -1 && field != "*")
                {
                    a.Add(
        ...
        /// </summary>
        /// <returns></returns>
        virtual protected string _BuildGroup()
        {
            if (_groupField.Count > 0)
            {
                return " GROUP BY " + String.Join(", ", _groupField.ToArray()) + " ";
            }
            else
            {
                return "";
        ...
        virtual protected string _ProtectIdentifiers(string identifier)
        {
            ...
            return idl + String.Join(idl + '.' + idl, a) + idl + alias;
        }
