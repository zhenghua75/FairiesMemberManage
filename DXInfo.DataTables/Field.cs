...
namespace DataTables
{
    ...
    public class Field
    {
        ...
        private Func<Dictionary<string, object>, object> _optsFormat;
        private Upload _upload;
        private Func<string, string> _xss;
        private Boolean _xssFormat = false;

        internal string OptsTable;
        internal string OptsValue;
        ...
        /// <param name="format">Formatting function (called for every option)</param>
        /// <returns>Self for chaining</returns>
        public Field Options(string table, string value, IEnumerable<string> label, Action<Query> condition = null, Func<Dictionary<string, object>, object> format = null)
        {
            _optsFn = null;
            OptsTable = table;
            OptsValue = value;
        ...
        internal List<Dictionary<string, object>> OptionsExec(Database db)
        {
            ...
            var fields = new List<string>(_optsLabel) { OptsValue };
            
            var rows = db.SelectDistinct(OptsTable, fields, _optsCond)
                .FetchAll();
            ...
            return rows.Select(row => new Dictionary<string, object>
            {
                {"value", row[OptsValue]},
                {"label", formatter(row)}
            }).OrderBy(x => _optsLabel.Count>1?x["value"]:x["label"]).ToList();
        }
