    ...
    public class DtRequest
    {
        ...
        public DtRequest(IEnumerable<KeyValuePair<string, string>> rawHttp)
        {
            ...
            if (http.ContainsKey("action"))
            {
                ...
                {
                    RequestType = RequestTypes.EditorVerify;
                    Data = http["data"] as Dictionary<string, object>;
                }
                else if (Action == "unverify")
                {
                    RequestType = RequestTypes.EditorUnVerify;
                    Data = http["data"] as Dictionary<string, object>;
                }
            }
            else if (http.ContainsKey("draw"))
            {
                ...
                string searchValue = search["value"]==null?"":search["value"].ToString();
                Search = new SearchT
                {
                    Value = searchValue,
                ...
                foreach (var item in http["columns"] as Dictionary<string, object>)
                {
                    ...
                    bool bArray = false;
                    KeyValuePair<string,object> aKey = colSearch.FirstOrDefault(f => f.Key == "array");
                    if(!string.IsNullOrEmpty(aKey.Key))
                    {
                        bArray = (Boolean)aKey.Value;
                    }
                    string columnName = column["name"]==null? "" : column["name"].ToString();
                    
                    string colSearchValue = colSearch["value"] == null ? "" : colSearch["value"].ToString();
                    Columns.Add(new ColumnT
                    {
                        ...
                        Groupable = column.Any(a => a.Key == "groupable") ? (Boolean)column["groupable"]:false,
                        Search = new SearchT
                        {
        ...
        {
            this.Columns.Add(new ColumnT(name, data));
        }
        /// <summary>
        /// 
        /// </summary>
        /// <param name="name"></param>
        /// <returns></returns>
        public ColumnT this[string name]
        {
            get
            {
                return Columns.Find(f => f.Name == name);
            }
        }
        /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
         * Private functions
        ...
        public class ColumnT
        {
            ...
            /// <param name="name"></param>
            /// <param name="data"></param>
            public ColumnT(string name,string data)
            {
                this.Data = data;
                this.Name = name;
                this.Search = new SearchT();
